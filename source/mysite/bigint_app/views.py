...
dir = /ABS-PATH-TO-BIGINT_CODE-DIRECTORY/
...
def div_random(request, dividend_size, divisor_size):
    ...
    os.chdir(dir)
    operands_file = str(uuid4()) + '.txt' # Write operands to file
    os.system('.' + dir + 'generate_operands ' + dividend_size + ' ' + divisor_size + ' > ' + operands_file)
...
def rule_random(request, divisor_size):
    if divisor_size.isdigit() == 0 or int(divisor_size) <= 0:
        return HttpResponse('Invalid input')
    os.chdir(dir)
    divisor_file = str(uuid4()) + '.txt'
    os.system('.' + dir + 'generate_operand ' + divisor_size + ' > ' + divisor_file) # Write rule to temp file
    ...
    if divisor_ not in ['1', '2', '3', '5']: # No div rule for 1, 2, 3 or 5
        try: # Check if exists
            ...
            rule_ = div_rule.objects.create( # Return JSON formatted rule results
                ...
                rule=rule,
                rule_size=length,
                negative_rule=True if rule[0] == '-' else False,
                date=timezone.now())
            rule_.save()
        return rule_
